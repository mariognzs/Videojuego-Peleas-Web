    ...
    EntityPlayerAttack = ig.Entity.extend({
      ...
      check: function (other) {
        ...
        if (other.invincible == false) {
          other.receiveDamage(320, this); // give damage when there is a hit
          other.damaged = true;

          if (other instanceof EntitySlime2 || other instanceof EntitySlime) {
            soundEffect = new ig.Sound("./media/SoundEffects/Pixabay/Enemies/slimeSpat.mp3");
            soundEffect.play();
          } else {
            switch (sound) {
              case 0:
