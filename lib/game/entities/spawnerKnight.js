ig.module("game.entities.spawnerKnight")
  .requires("impact.entity", "game.entities.barrerKnight", "game.entities.bossMage")
  .defines(function () {
    // Subclase de ig.Enitity
    EntitySpawnerKnight = ig.Entity.extend({
      ...
      update: function () {
        ...
        if (ig.Timer.timeScale == 1) {
          if (this.bossCheck.health < 320 && this.spawnTimer <= 0) {
            ig.game.spawnEntity("EntityBarrerKnight", this.pos.x, this.pos.y, true);
            // Alrededor de 15 segundos 1000
            this.spawnTimer = 5750;
          } else {
            if (this.bossCheck.health < 320 && this.bossCheck.health > 0) {
              this.spawnTimer--;
