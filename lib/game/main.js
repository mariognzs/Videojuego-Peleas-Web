ig.module("game.main")
  ...
  .defines(function () {
    MyGame = ig.Game.extend({
      ...
      font2: new ig.Font("media/healthFont.png"),
      gravity: 180,
      player: null,
      healthBar: null,
      // enemy: null,
      ...
      title4: new ig.Image("media/win.png"),

      boss: null,
      bossHealthBar: new ig.Image("media/barBoss.png"),
      bossInvincibleBar: new ig.Image("media/barBossInvincible.png"),
      win: false,
      ...
      init: function () {
        ...
        try {
          this.boss = ig.game.getEntitiesByType(EntityBossMage)[0];
        } catch (error) {
          console.log(error);
        }
      ...
      update: function () {
        ...
        if (ig.input.pressed("restart") && this.player.health < 1) {
          ...
          ig.Timer.timeScale = 1;
          return;
        } else if (ig.input.pressed("restart") && this.win == true) {
          ig.system.setGame(MyTitle);
          this.win = false;
          this.boss = null;
          ig.Timer.timeScale = 1;
      ...
      draw: function () {
        ...
        try {
          if (this.boss != null) {
            if (this.boss.health <= 0 && this.boss.dead == true) {
              ig.music.stop();

              if (this.win == false) {
                soundEffect = new ig.Sound(
                  "./media/SoundEffects/Pixabay/win.ogg",
                  false
                );
                soundEffect.play();
                this.win = true;
              ...
              this.title4.draw(cx - this.title.width / 2, 0);
              ...
              var bossHealth = "Zoltar the dark mage";
              this.font.draw(bossHealth, 104, 200, ig.Font.ALIGN.RIGHT);

              var healthPercentBoss = this.boss.health / 440;
              var barWidthBoss;

              if (this.boss.health > 0) {
                barWidthBoss = 300 * healthPercentBoss;

                if (this.boss.invincible == true) {
                  this.bossInvincibleBar.draw(10, 212, 0, 0, barWidthBoss);
                } else if (this.boss.invincible == false) {
                  this.bossHealthBar.draw(10, 212, 0, 0, barWidthBoss);
                }
              }
            }
          }
        ...
        if (this.font) {
          ...
          if (this.player.health > 0) {
            ...
            try {
              this.font2.draw(
                Math.floor((this.player.health * 100) / 50) + "%",
                ig.system.width / 2,
    ...
    MyTitle = ig.Game.extend({
      ...
      init: function () {
        ...
        try {
          ig.music.add(
            "media/SoundEffects/Pixabay/BgMusic/bgIntro.ogg",
            "intro"
          );
        } catch (error) {
          console.log(error);
